...
contract Corn is ERC777, IERC777Recipient, ReentrancyGuard {
    ...
    function getAddressMaturityMultiplier(address targetAddress) public view returns(uint256) {
        ...
        uint256 targetBlockNumber = targetFarm.blockNumber.add(_startMaturityBoost);
        ...
        uint256 blockDiff = block.number.sub(targetBlockNumber).mul(maturityBoostExtension).div(_endMaturityBoost).add(_percentMultiplier); 
